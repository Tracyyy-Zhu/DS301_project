"""Run a hyperparameter search on a BERT model fine-tuned on BoolQ.

Example usage:
    python bert_hyper.py BoolQ/
"""
import argparse
import boolq
import data_utils
...
parser = argparse.ArgumentParser(
    description="Run hyperparameter search for finetuning a BERT model on the BoolQ dataset."
)
parser.add_argument(
    "data_dir",
    type=str,
...
trainer = transformers.Trainer(
                               ...
                               tokenizer=tokenizer,
                               model_init=bert_finetune.model_init,
                               compute_metrics=bert_finetune.compute_metrics,
)
...
best_result = trainer.hyperparameter_search(hp_space=new_hp_space,
                                            ...
                                            direction='maximize',
                                            backend='ray',
                                            scheduler=HyperBandScheduler(max_t=5),
                                            mode='max',
                                            log_to_file=True,
